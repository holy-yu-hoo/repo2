...
class Universe(models.Model):
	title = models.CharField('universe title', unique = True, max_length = 200, blank = True)
	...
	class Meta:
		...
		constraints = [
			models.UniqueConstraint(fields = ['title', 'author'], name = 'universe_author_unique'),
		]


class Character(models.Model):
	name = models.CharField('character name', unique = True, max_length = 200, blank = True)
	...
	@canonic.setter
	def canonic(self, value: bool):
		if value not in (True, False):
			raise ValueError('Canonic must be True or False')
		self.canon = value

	def set_universe_by_tile(self, universe_tile: str):
		try:
	...
	class Meta:
		constraints = (
			models.UniqueConstraint(fields = ['name', 'universe'], name = 'character_universe_unique'),
		)
		indexes = (
